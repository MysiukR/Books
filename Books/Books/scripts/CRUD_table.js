...
$(document).ready(function () {
    $("#submit").click(function () {
        ...
        if (!year.length && pages.indexOf("a") == -1 && $("#txtName").val() != "" && $("#txtAuthor").val() != "" && $("#txtPages").val() != "" && $("#txtYear").val() != "") {
            ...
            var markup = "<tr><td>" + name + "</td><td>" + author + "</td><td>" + pages + "</td><td>" + year + "</td><td> <input class='btn btn-danger' type='submit' data-toggle='modal' data-target='#myModal1' value='Remove' id='submit' name='submit'/></td></tr>";
            $("table tbody").append(markup);

            $.ajax({
                type: "POST",
                url: "/Home/InsertInDatabase",
                data: "{name:'" + name + "',author:'" + author + "',pages:'" + pages + "',year:'" + year + "'}",
